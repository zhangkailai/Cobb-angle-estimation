...
def get_rpn_testbatch(roidb, cfg):
    ...
    imgs, roidb, name = get_image(roidb, cfg)
    im_array = imgs
    ...
    return data, label, im_info, name
...
def get_rpn_batch(roidb, cfg):
    ...
    if roidb[0]['gt_classes'].size > 0:
        ...
        gt_points = np.empty((roidb[0]['points'].shape[0], 8),dtype=np.float32)
        gt_boxes[:, 0:4] = roidb[0]['boxes'][gt_inds, :]
        gt_boxes[:, 4] = roidb[0]['gt_classes'][gt_inds]
        gt_points[:,0:8] = roidb[0]['points'][gt_inds, :]
    else:
        gt_boxes = np.empty((0, 5), dtype=np.float32)
        gt_points = np.empty((0, 8), dtype=np.float32)
...
def assign_anchor(feat_shape, gt_boxes, gt_points, im_info, cfg, feat_stride=16,
    ...
    points_targets = np.zeros((len(inds_inside),8), dtype=np.float32)

    if gt_boxes.size > 0:
        bbox_targets[:] = bbox_transform(anchors, gt_boxes[argmax_overlaps, :4])
        points_targets[:] = gt_points[argmax_overlaps,:8]

    bbox_weights = np.zeros((len(inds_inside), 4), dtype=np.float32)
    bbox_weights[labels == 1, :] = np.array(cfg.TRAIN.RPN_BBOX_WEIGHTS)

    points_weights = np.zeros((len(inds_inside), 8), dtype=np.float32)
    points_weights[labels == 1,:] = np.array(cfg.TRAIN.RPN_POINTS_WEIGHTS)
    if DEBUG:
    ...
    points_targets = _unmap(points_targets, total_anchors, inds_inside, fill=0)
    points_weights = _unmap(points_weights, total_anchors, inds_inside, fill=0)
    if DEBUG:
    ...
    points_targets = points_targets.reshape((1, feat_height, feat_width, A * 8)).transpose(0, 3, 1, 2)
    points_weights = points_weights.reshape((1, feat_height, feat_width, A * 8)).transpose((0, 3, 1, 2))


    label = {'label': labels,
             'bbox_target': bbox_targets,
             'bbox_weight': bbox_weights}
           #  'points_target': points_targets,
           #  'points_weight': points_weights}
    return label
