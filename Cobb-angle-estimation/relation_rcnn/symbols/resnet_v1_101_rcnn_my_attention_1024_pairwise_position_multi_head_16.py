...
class resnet_v1_101_rcnn_my_attention_1024_pairwise_position_multi_head_16(resnet_v1_101_rcnn_base):
    def __init__(self):
        """
...
#            rois, label, bbox_target, bbox_weight = mx.sym.Custom(rois=rois, gt_boxes=gt_boxes_reshape,
        ...
        matrix_list1 = self.attention_module_multi_head(fc_new_1, position_embedding, nongt_dim=nongt_dim, fc_dim=16, feat_dim=1024, index=0, dim=(1024, 1024, 1024))
        for i in range(1,8):
            attention_matrix1 = self.attention_module_multi_head(fc_new_1, position_embedding, nongt_dim=nongt_dim, fc_dim=16, feat_dim=1024, index=i, dim=(1024, 1024, 1024))
...
#        attention_1 = mx.sym.mean(matrix_list1, axis=2)
        ...
        matrix_list2 = self.attention_module_multi_head(fc_new_2, position_embedding, nongt_dim=nongt_dim, fc_dim=16, feat_dim=1024, index=8, dim=(1024, 1024, 1024))
        for i in range(1,8):
            attention_matrix2 = self.attention_module_multi_head(fc_new_2, position_embedding, nongt_dim=nongt_dim, fc_dim=16, feat_dim=1024, index=i+8, dim=(1024, 1024, 1024))
