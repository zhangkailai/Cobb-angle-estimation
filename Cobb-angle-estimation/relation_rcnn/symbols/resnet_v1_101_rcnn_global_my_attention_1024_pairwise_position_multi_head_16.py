...
class resnet_v1_101_rcnn_global_my_attention_1024_pairwise_position_multi_head_16(resnet_v1_101_rcnn_base):
    def __init__(self):
        """
    ...
    def extract_position_embedding(position_mat, feat_dim, wave_length=1000):
        ...
        # position_mat, [num_rois, nongt_dim, 4, 1]
    ...
    def attention_module_multi_head(self, global_feat, roi_feat, position_embedding,
                                    nongt_dim, fc_dim, feat_dim,
                                    dim=(1024, 1024, 1024), index=1):
#                                    group=16, index=1):
...
#        dim_group = (dim[0] / group, dim[1] / group, dim[2] / group)
        total_feat = mx.sym.broadcast_plus(lhs=roi_feat, rhs=global_feat)
        nongt_roi_feat = mx.symbol.slice_axis(data=total_feat, axis=0, begin=0, end=nongt_dim)
        # nongt_roi_feat, [nongt_dim, feat_dim]
        ...
        position_feat_1 = mx.sym.FullyConnected(name='pair_pos_fc1_' + str(index),
                                                data=position_embedding_reshape,num_hidden=1)
#                                                num_hidden=fc_dim)
        position_feat_1_relu = mx.sym.Activation(data=position_feat_1, act_type='relu')
        # position_feat_1_relu, [num_rois * nongt_dim, dim[2]]
        # aff_weight, [num_rois, nongt_dim, dim[2]]
#        aff_weight = mx.sym.Reshape(position_feat_1_relu, shape=(-1, nongt_dim, fc_dim))
        # aff_weight, [num_rois, dim[2], nongt_dim]
#        aff_weight = mx.sym.transpose(aff_weight, axes=(0, 2, 1))
        aff_weight = mx.sym.Reshape(position_feat_1_relu, shape=(-1, nongt_dim))
...
#        aff_softmax_reshape = mx.sym.Reshape(aff_softmax, shape=(-3, -2))
    ...
    def get_symbol(self, cfg, is_train=True):
        ...
        if is_train:
            ...
            gt_points = mx.sym.Variable(name="gt_points")
        ...
        if is_train:
            ...
            gt_points_reshape = mx.sym.Reshape(data=gt_points, shape=(-1,8), name='gt_points_reshape')
            rois, label, bbox_target, bbox_weight, points_target, points_weight = mx.sym.Custom(rois=rois, gt_boxes=gt_boxes_reshape, gt_points=gt_points_reshape,
#            rois, label, bbox_target, bbox_weight = mx.sym.Custom(rois=rois, gt_boxes=gt_boxes_reshape,
        ...
        conv_global = mx.symbol.Pooling(data=conv_new_1_relu, kernel=(2,2), pool_type='max', global_pool=True)
        global_feature = mx.symbol.FullyConnected(name='fc_global', data=conv_global, num_hidden=1024)
        # 2 fc
        fc_new_1 = mx.symbol.FullyConnected(name='fc_new_1', data=roi_pool, num_hidden=1024)
        # attention, [num_rois, feat_dim]
        matrix_list1 = self.attention_module_multi_head(global_feature, fc_new_1, position_embedding, nongt_dim=nongt_dim, fc_dim=16, feat_dim=1024, index=0, dim=(1024, 1024, 1024))
        for i in range(1,16):
            attention_matrix1 = self.attention_module_multi_head(global_feature, fc_new_1, position_embedding, nongt_dim=nongt_dim, fc_dim=16, feat_dim=1024, index=i, dim=(1024, 1024, 1024))
            matrix_list1 = mx.sym.concat(matrix_list1, attention_matrix1, dim=2)            
            

        attention_1 = mx.sym.max(matrix_list1, axis=2)
#        attention_1 = mx.sym.mean(matrix_list1, axis=2)
        ...
        matrix_list2 = self.attention_module_multi_head(global_feature, fc_new_2, position_embedding, nongt_dim=nongt_dim, fc_dim=16, feat_dim=1024, index=16, dim=(1024, 1024, 1024))
        for i in range(1,16):
            attention_matrix2 = self.attention_module_multi_head(global_feature, fc_new_2, position_embedding, nongt_dim=nongt_dim, fc_dim=16, feat_dim=1024, index=i+16, dim=(1024, 1024, 1024))
            matrix_list2 = mx.sym.concat(matrix_list2, attention_matrix2, dim=2)


        attention_2 = mx.sym.max(matrix_list2, axis=2)
#        attention_2 = mx.sym.mean(matrix_list2, axis=2)
        ...
        points_pred = mx.symbol.FullyConnected(name='points_pred', data=fc_all_2_relu, num_hidden=8)
        points_pred = mx.sym.sigmoid(points_pred)

        if is_train:
            if cfg.TRAIN.ENABLE_OHEM:
                labels_ohem, bbox_weights_ohem, points_weights_ohem = mx.sym.Custom(op_type='BoxAnnotatorOHEM', num_classes=num_classes,
                                                               num_reg_classes=num_reg_classes,
                                                               roi_per_img=cfg.TRAIN.BATCH_ROIS_OHEM,
                                                               cls_score=cls_score, bbox_pred=bbox_pred, points_pred=points_pred, labels=label,
                                                               bbox_targets=bbox_target, bbox_weights=bbox_weight, points_targets=points_target, points_weights=points_weight)
                ...
                points_loss_ = points_weights_ohem * mx.sym.smooth_l1(name='points_loss_', scalar=1.0,
                                                                  data=(points_pred-points_target)) 

                points_loss = mx.sym.MakeLoss(name='points_loss', data=points_loss_, grad_scale=1.0/cfg.TRAIN.BATCH_ROIS_OHEM)
            else:
            ...
            points_loss = mx.sym.Reshape(data=points_loss, shape=(cfg.TRAIN.BATCH_IMAGES, -1, 8), name='points_loss_reshape')

#            group = mx.sym.Group([rpn_cls_prob, rpn_bbox_loss, cls_prob, bbox_loss, mx.sym.BlockGrad(rcnn_label)])
            group = mx.sym.Group([rpn_cls_prob, rpn_bbox_loss, cls_prob, bbox_loss, points_loss, mx.sym.BlockGrad(rcnn_label)])
        else:
            ...
            points_pred = mx.sym.Reshape(points_pred, shape=(cfg.TEST.BATCH_IMAGES, -1, 8),
                                       name='points_pred_reshape')
 
            #group = mx.sym.Group([rois, cls_prob, bbox_pred, fc_new_1, aff_softmax_0, aff_softmax_1, aff_softmax_2, aff_softmax_3])
            group = mx.sym.Group([rois, cls_prob, bbox_pred, points_pred, attention_1, attention_2])
#            group = mx.sym.Group([rois, cls_prob, bbox_pred, attention_1, attention_2])
    ...
    def init_weight_rcnn(self, cfg, arg_params, aux_params):
        ...
        arg_params['fc_global_weight'] = mx.random.normal(0, 0.01, shape=self.arg_shape_dict['fc_global_weight'])
        arg_params['fc_global_bias'] = mx.nd.zeros(shape=self.arg_shape_dict['fc_global_bias'])
        ...
        arg_params['points_pred_weight'] = mx.random.normal(0, 0.01, shape=self.arg_shape_dict['points_pred_weight'])
        arg_params['points_pred_bias'] = mx.nd.zeros(shape=self.arg_shape_dict['points_pred_bias'])

        for idx in range(0, 32):
            self.init_weight_attention_multi_head(cfg, arg_params, aux_params, index=idx)
