...
class BoxAnnotatorOHEMOperator(mx.operator.CustomOp):
    ...
    def forward(self, is_train, req, in_data, out_data, aux):
        ...
        cls_score    = in_data[0]
        bbox_pred    = in_data[1]
        points_pred  = in_data[2]
        labels       = in_data[3].asnumpy()
        bbox_targets = in_data[4]
        bbox_weights = in_data[5]
        points_targets = in_data[6]
        points_weights = in_data[7]
        ...
        per_roi_loss_points = points_weights * mx.nd.smooth_l1((points_pred-points_targets), scalar=1.0)
        per_roi_loss_points = mx.nd.sum(per_roi_loss_points, axis=1).asnumpy()

        top_k_per_roi_loss = np.argsort(per_roi_loss_cls + per_roi_loss_bbox + per_roi_loss_points)
        labels_ohem = labels
        ...
        points_weights_ohem = points_weights.asnumpy()
        points_weights_ohem[top_k_per_roi_loss[::-1][self._roi_per_img:]] = 0

        labels_ohem = mx.nd.array(labels_ohem)
        bbox_weights_ohem = mx.nd.array(bbox_weights_ohem)
        points_weights_ohem = mx.nd.array(points_weights_ohem)

        for ind, val in enumerate([labels_ohem, bbox_weights_ohem, points_weights_ohem]):
...
class BoxAnnotatorOHEMProp(mx.operator.CustomOpProp):
    ...
    def list_arguments(self):
        return ['cls_score', 'bbox_pred', 'points_pred', 'labels', 'bbox_targets', 'bbox_weights', 'points_targets', 'points_weights']

    def list_outputs(self):
        return ['labels_ohem', 'bbox_weights_ohem', 'points_weights_ohem']

    def infer_shape(self, in_shape):
        labels_shape = in_shape[3]
        bbox_weights_shape = in_shape[5]
        points_weights_shape = in_shape[7]

        return in_shape, \
               [labels_shape, bbox_weights_shape, points_weights_shape]
