...
class ProposalTargetOperator(mx.operator.CustomOp):
    ...
    def forward(self, is_train, req, in_data, out_data, aux):
        ...
        if self._batch_rois == -1 or self._batch_rois == -2:
            ...
            rois, labels, bbox_targets, bbox_weights, points_targets, points_weights = \
                sample_rois_v2(all_rois, self._num_classes, self._cfg, gt_boxes=gt_boxes, gt_points=gt_points)
        else:
            rois, labels, bbox_targets, bbox_weights = \
                sample_rois(all_rois, fg_rois_per_image, rois_per_image, self._num_classes, self._cfg, gt_boxes=gt_boxes)
        ...
        for ind, val in enumerate([rois, labels, bbox_targets, bbox_weights, points_targets, points_weights]):
...
class ProposalTargetProp(mx.operator.CustomOpProp):
    ...
    def list_arguments(self):
        return ['rois', 'gt_boxes', 'gt_points']

    def list_outputs(self):
        return ['rois_output', 'label', 'bbox_target', 'bbox_weight', 'points_target', 'points_weight']

    def infer_shape(self, in_shape):
        rpn_rois_shape = in_shape[0]
        gt_boxes_shape = in_shape[1]
        gt_points_shape = in_shape[2]
        ...
        points_target_shape = (rois, 8)
        points_weight_shape = (rois, 8)
        return [rpn_rois_shape, gt_boxes_shape, gt_points_shape], \
               [output_rois_shape, label_shape, bbox_target_shape, bbox_weight_shape, points_target_shape, points_weight_shape]
