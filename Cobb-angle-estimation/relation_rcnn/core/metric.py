...
def get_rcnn_names(cfg):
    pred = ['rcnn_cls_prob', 'rcnn_bbox_loss', 'rcnn_points_loss']
    label = ['rcnn_label', 'rcnn_bbox_target', 'rcnn_bbox_weight','rcnn_points_target','rcnn_points_weight']
...
class RCNNL1LossMetric(mx.metric.EvalMetric):
    ...
    def update(self, labels, preds):
        ...
        points_loss = preds[self.pred.index('rcnn_points_loss')].asnumpy()
        if self.ohem:
        ...
        self.sum_metric += np.sum(bbox_loss)
        self.sum_metric += np.sum(points_loss)
