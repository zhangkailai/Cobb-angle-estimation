...
class TestLoader(mx.io.DataIter):
    ...
    def next(self):
        ...
        if self.iter_next():
            ...
            self.lock.release()
            data, im_info, name = self.get_batch(cur_from)
            return im_info, mx.io.DataBatch(data=data, label=self.label,
                   pad=self.getpad(cur_from), index=self.getindex(cur_from),
                   provide_data=self.provide_data_func(data), provide_label=self.provide_label_func(data)), name
        else:
            self.lock.release()
            raise StopIteration
    ...
    def get_batch(self, cur_from=None):
        ...
        if self.has_rpn:
            data, label, im_info, file_name = get_rpn_testbatch(roidb, self.cfg)
        else:
...
class AnchorLoader(mx.io.DataIter):
    ...
    def __init__(self, feat_sym, roidb, cfg, batch_size=1, shuffle=False, ctx=None, work_load_list=None,
        ...
        if config.TRAIN.END2END:
            self.data_name = ['data', 'im_info', 'gt_boxes', 'gt_points']
        else:
            self.data_name = ['data']
        self.label_name = ['label', 'bbox_target', 'bbox_weight']#, 'points_target', 'points_weight']
    ...
    def infer_shape(self, max_data_shape=None, max_label_shape=None):
        ...
        label = assign_anchor(feat_shape[0], np.zeros((0, 5)), np.zeros((0,8)), im_info, self.cfg,
    ...
    def parfetch(self, iroidb):
        ...
        data['gt_points'] = label['gt_points'][np.newaxis, :, :]
        # assign anchor for label
        label = assign_anchor(feat_shape, label['gt_boxes'], label['gt_points'], data['im_info'], self.cfg,
